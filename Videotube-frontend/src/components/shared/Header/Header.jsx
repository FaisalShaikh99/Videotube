      ...
      <header className="fixed top-0 left-0 right-0 z-50 h-16 transition-all duration-300 backdrop-blur-md bg-white/80 dark:bg-slate-900/80 border-b border-gray-200 dark:border-gray-800">
        <div className="max-w-[1920px] mx-auto px-4 h-full flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
             ...
             <button 
                onClick={toggleTheme}
                className="p-2 rounded-full text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors focus:ring-2 focus:ring-indigo-500/30"
                title="Toggle Theme"
             >
             ...
             <div className="flex md:hidden items-center gap-2">
                ...
                {!isAuthenticated && (
                    <Link to="/login">
                        <Button variant="primary" size="sm" className="px-5 py-1.5 h-auto text-sm">Login</Button>
                    </Link>
                )}
                 {isAuthenticated && (
                     <Link to="/profile">
                      <Avatar size="xs" src={user?.avatar} className="w-8 h-8" />
                     </Link>
                 )}
             </div>
          </div>
